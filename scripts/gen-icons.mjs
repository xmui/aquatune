...
const svgFavicon = buildSvg({ scale: 0.96, withDarkMedia: true });
out('icon.svg', Buffer.from(svgFavicon));

const svgLight = buildSvg({ scale: 0.96 });          // big, with just a sliver of edge margin
const svgDark = buildSvg({ scale: 0.96, dark: true });
const svgMask = buildSvg({ scale: 0.78 });          // tighter so it stays inside the maskable circle
const svgMaskDark = buildSvg({ scale: 0.78, dark: true });
